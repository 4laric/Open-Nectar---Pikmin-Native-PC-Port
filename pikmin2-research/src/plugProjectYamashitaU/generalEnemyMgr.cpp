...
// Additive Bomb birth hook (#677, #616 provider, #577 payload).
// Notifies the port Bomb::Mgr birth seam when the engine creates a
// Bomb/BombOtakara manager, so #577 payload births route to live carriers
// instead of numeric stubs. Weak default is a no-op; the pc_port provider
// (pc_p2_bomb_mgr_birth) overrides it where linked. No decomp arm is
// reordered or removed; no save/ledger semantics change here.
__attribute__((weak)) void pc_p2_bomb_birth_hook_notify(int enemyID);
static void pc_p2_bomb_birth_hook_maybe_notify(int enemyID)
{
    if (pc_p2_bomb_birth_hook_notify) pc_p2_bomb_birth_hook_notify(enemyID);
}
...
void GeneralEnemyMgr::createEnemyMgr(u8 viewNum, int enemyID, int limit)
{
	...
	case EnemyTypeID::EnemyID_Bomb:
		...
		pc_p2_bomb_birth_hook_maybe_notify(enemyID);
		break;
