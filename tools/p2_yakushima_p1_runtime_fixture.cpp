...
// P1 yakushima run layout, enforces captain safety #632 on every idle tick, and
// checks each surface-session boundary against actual engine capability.
// Boundaries the engine cannot perform (overworld course boot, day advance,
// save serializer, receipt ledger, exit/reentry) are reported UNSUPPORTED and
// the run FAILS CLOSED: no PASS marker is ever emitted without genuine
// observation. Diagnostic engine facts are labelled as such and prove nothing
// about the surface session.
//
// Replacement-main convention mirrors tools/p2_kurage_runtime.cpp and
// tools/p2_cave_guarded_boot_fixture.cpp (scenario main instead of
...
// lane runner links this TU against the private pikmin_pc graph without
// editing shared build files.
#include <SDL2/SDL.h>
#include <GL/gl.h>
#include "App.h"
#include "Node.h"
...
bool sForceCaptainDown = false; // env P2_YAKUSHIMA_P1_FORCE_CAPTAIN_DOWN=1: negative-path test only
...
int guardSelfTest() {
    ...
    for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); ++i) {
        ...
        if (down != rows[i].expectDown) {
            std::printf("FAIL YAKUSHIMA_P1_RUNTIME selftest row=%d orima=%d dead=%d hp=%.3f got=%d want=%d\n",
    ...
    std::printf("P2_YAKUSHIMA_P1_SELFTEST_PASS rows=%d\n", int(sizeof(rows) / sizeof(rows[0])));
    std::fflush(stdout);
    return 0;
}

class YakushimaP1RuntimeApp final : public PlugPikiApp {
    ...
    void unsupported(const char* boundary, const char* reason) {
        std::printf("P2_YAKUSHIMA_P1_UNSUPPORTED boundary=%s reason=%s\n", boundary, reason);
        std::fflush(stdout);
    }
public:
    int idle() override {
        ...
        if (++frames > 30000) {
            std::printf("FAIL YAKUSHIMA_P1_RUNTIME timeout observed=%d\n", observed);
            std::fflush(stdout);
            std::_Exit(2);
        }
        if (!naviMgr || !tekiMgr || !pikiMgr) {
            if (frames % 600 == 0) {
                std::printf("P2_YAKUSHIMA_P1_WAIT frames=%d navi_mgr=%d teki_mgr=%d piki_mgr=%d\n",
        ...
        if (!n) {
            if (frames % 600 == 0) {
                std::printf("P2_YAKUSHIMA_P1_WAIT frames=%d navi=0\n", frames);
                std::fflush(stdout);
            }
            return result;
        }
        ...
        if (!factsLogged && observed >= 120) {
            factsLogged = true;
            // Diagnostic engine facts ONLY: they prove the harness boots and
            // observes a live room scene. They say nothing about the yakushima
            // surface session, day, saves, receipts, or reentry.
            std::printf("P2_YAKUSHIMA_P1_ENGINE_FACT observed=%d squad_alive=%d diagnostic_only=1\n",
                        observed, alivePikis());
            std::fflush(stdout);
            // Each required boundary is checked against engine capability and
            // fails closed: this port has no overworld course boot, no day
            // advance, no save serializer, no receipt ledger, and no
            // exit/reentry path, so none can be observed here.
            unsupported("boot_yakushima_surface", "no-overworld-course-boot-in-port");
            ...
            std::printf("FAIL YAKUSHIMA_P1_RUNTIME boundaries_unobservable=5 observed=%d\n", observed);
            std::fflush(stdout);
            std::_Exit(1);
        }
        if (observed % 600 == 0 && waitMarks < 48) {
            ++waitMarks;
            std::printf("P2_YAKUSHIMA_P1_WAIT observed=%d squad_alive=%d\n", observed, alivePikis());
            std::fflush(stdout);
        }
        return result;
    }
...
int main(int argc, char** argv) {
    ...
    if (sGuardNegativeTest) {
        ...
        std::printf("FAIL YAKUSHIMA_P1_RUNTIME negative test did not trip\n");
        std::fflush(stdout);
        return 1;
    }
    const char* force = std::getenv("P2_YAKUSHIMA_P1_FORCE_CAPTAIN_DOWN");
    ...
    if (!pc_pikipelago_room_preview()) {
        std::printf("FAIL YAKUSHIMA_P1_RUNTIME requires --experimental-pikmin2-room\n");
        std::fflush(stdout);
        return 3;
    }
    if (!pc_window_init("P2 Yakushima P1 runtime fixture", 960, 540)) return 3;
    pc_window_center();
    {
        ...
        std::printf("P2_YAKUSHIMA_P1_WINDOW size=%dx%d pos=%d,%d display=%dx%d centered=%d\n",
    ...
    nodeMgr = new NodeMgr();
    gsys->run(new YakushimaP1RuntimeApp());
    return 0;
}
